  ...
  <?php
	...
	for ($x = 0; $x <= 0; $x++) {
		if (isset($_POST['submit'])){
			...
			if ($username == "" OR $entry == "" OR $reference == "") {
				echo("Error! Blank Fields!");
			}
			else if ($username == " " OR $entry == " " OR $reference == " ") { // CONTINUE THIS CODE
				echo("Error! Spaces included!");
			}
			else {
								   ...
								   echo("<p>Error in retreiving row count!</p>");
									...
									function insertData() // Use this function to insert user details into user table!
									{
										try
										{
											...
											$resReference = preg_replace($patternsReference, "", $reference);
							...
							if($conn == true)
								try
								{
									...
									else
									{
										...
										if ($category == $nations) {
											echo("<td><h2>Category: Nations</h2></td>");
										}
										else if ($category == $conflicts) {
											echo("<td><h2>Category: Conflicts</h2></td>");
										}
										else if ($category == $science) {
											echo("<td><h2>Category: Science</h2></td>");
										}
									}
									sqlsrv_close($conn);
								}
						...
						try
						{
							...
							if($conn == true)
								try
								{
									...
									echo("<td><textarea name='entry' id='entryEdit' rows='60' cols='30'>");
						...
						try
						{
							...
							if($conn == true)
								try
								{
									...
									echo("<td><textarea name='reference' id='referenceEdit' rows='60' cols='30'>");
